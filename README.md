...
Level-up now includes a manual rich spell review whenever spell capability changes. Current valid prepared spells are preserved and prechecked, newly accessible spell levels are selectable but never auto-prepared, and searchable/filterable cards expand to the same rules details used by Spellbook. Circle always-prepared spells and Chthonic species grants are shown separately, read-only, and excluded from the normal Druid preparation count; Magician's extra cantrip also remains separate.
...
### Iteration 3.5C advancement completion

Direct creation now adds a dedicated Advancement step at Druid levels 4–8: level 4 is required for levels 4–7 and both level 4 and level 8 are required at level 8. ASI supports a verified +2 or split +1/+1 with a rules-layer score cap of 20. The typed General Feat registry enables Resilient, Skill Expert, and non-repeatable Tough when eligible; unavailable or unverified entries cannot be selected. Level-up requests only the newly crossed milestone and preserves live session state.

Stored user Druids missing historical milestones receive ordered, recoverable required-choice markers rather than an inferred ASI. The reference character explicitly records Wisdom +2 at levels 4 and 8. Choices are permanent in this iteration; respec, multiclassing, levels above 8, subclass changes, and further feats remain deferred. Destructive actions now share the bright-scarlet `#d91e18` danger token with accessible hover, active, focus, and disabled companions. See [the Iteration 3.5C advancement notes](docs/iteration-3-5c-druid-advancement-feats-asi.md).
